        ...
        private Trace BoxCastToFloor(float distance = 0.05f, float extentModifier = 1.0f)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (_hitCache[i].normal.y <= SurfPhysics.SurfSlope && _hitCache[i].normal.y > 0)
                {
                    return new Trace
                    {
                        HitCollider = _hitCache[i].collider,
                        HitPoint = _hitCache[i].point,
                        PlaneNormal = _hitCache[i].normal,
                        Distance = _hitCache[i].distance,
                        IsShit = Tracer.HitIsShit(_hitCache[i])
                    };
                }
            ...
            return new Trace
            {
                ...
                PlaneNormal = bestHit.normal,
                Distance = bestHit.distance,
                IsShit = Tracer.HitIsShit(bestHit)
            };
        }
